...
class GlobalCostmapBuilder(Node):
    ...
    def local_costmap_callback(self, msg):
        ...
        if (local_min_x < self.global_origin_x or
            local_max_x > self.global_origin_x + self.global_width * self.global_resolution or
            local_min_y < self.global_origin_y or
            local_max_y > self.global_origin_y + self.global_height * self.global_resolution):
        ...
        for y in range(msg.info.height):
            for x in range(msg.info.width):
                ...
                global_x = int((x * msg.info.resolution + local_origin_x - self.global_origin_x) / self.global_resolution)
                global_y = int((y * msg.info.resolution + local_origin_y - self.global_origin_y) / self.global_resolution)
    ...
    def resize_global_costmap(self, min_x, max_x, min_y, max_y):
        ...
        new_global_grid[offset_y:offset_y + self.global_height, offset_x:offset_x + self.global_width] = self.global_grid
